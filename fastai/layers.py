...
__all__ = ['module', 'Identity', 'Lambda', 'PartialLambda', 'Flatten', 'ToTensorBase', 'View', 'ResizeBatch',
           'Debugger', 'sigmoid_range', 'SigmoidRange', 'AdaptiveConcatPool1d', 'AdaptiveConcatPool1d', 'PoolType',
           ...
           'MaxPool', 'AvgPool', 'trunc_normal_', 'Embedding', 'SelfAttention', 'PooledSelfAttention1d',
...
class AdaptiveConcatPool1d(Module):
    ...
    def __init__(self, size=None):
        self.size = size or 1
        self.ap = nn.AdaptiveAvgPool1d(self.size) # TODO input check
        self.mp = nn.AdaptiveMaxPool1d(self.size)
    def forward(self, x): return torch.cat([self.mp(x), self.ap(x)], 1)
...
def adaptive_pool(pool_type):
    return nn.AdaptiveAvgPool1d if pool_type=='Avg' else nn.AdaptiveMaxPool1d if pool_type=='Max' else AdaptiveConcatPool1d

# Cell
class PoolFlatten(nn.Sequential):
    "Combine `nn.AdaptiveAvgPool1d` and `Flatten`."
...
# Cell
@delegates(nn.BatchNorm1d)
def BatchNorm(nf, ndim=2, norm_type=NormType.Batch, **kwargs):
    "BatchNorm layer with `nf` features and `ndim` initialized depending on `norm_type`."
    return _get_norm('BatchNorm', nf, ndim, zero=norm_type==NormType.BatchZero, **kwargs)

# Cell
@delegates(nn.InstanceNorm1d)
...
class ConvLayer(nn.Sequential):
    ...
    @delegates(nn.Conv1d)
    def __init__(self, ni, nf, ks=3, stride=1, padding=None, bias=None, ndim=1, norm_type=NormType.Batch, bn_1st=True,
...
# Cell
class PooledSelfAttention1d(Module):
    "Pooled self attention layer for 1d."
    def __init__(self, n_channels):
    ...
    def forward(self, x):
        n_ftrs = x.shape[2]
        f = self.query(x).view(-1, self.n_channels//8, n_ftrs) # TODO : necessary?
        g = F.max_pool1d(self.key(x),   [2]).view(-1, self.n_channels//8, n_ftrs//4)
        h = F.max_pool1d(self.value(x), [2]).view(-1, self.n_channels//2, n_ftrs//4)
        beta = F.softmax(torch.bmm(f.transpose(1, 2), g), -1)
        o = self.out(torch.bmm(h, beta.transpose(1,2)).view(-1, self.n_channels//2, x.shape[2]))
...
class PixelShuffle_ICNR(nn.Sequential):
    ...
    def __init__(self, ni, nf=None, scale=2, blur=False, norm_type=NormType.Weight, act_cls=defaults.activation):
        ...
        if blur: layers += [nn.ReplicationPad1d((1,0)), nn.AvgPool1d(2, stride=1)]
        super().__init__(*layers)
